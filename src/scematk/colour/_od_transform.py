...
def grey_to_od(image: Array, clip = True) -> Array:
...
def gray_to_od(image: Array, clip = True) -> Array:
    return grey_to_od(image, clip)

def od_to_grey(image: Array, as_ubyte = True) -> Array:
    assert image.ndim == 2, "Input image must be 2D"
...
def od_to_gray(image: Array, as_ubyte = True) -> Array:
    return od_to_grey(image, as_ubyte)

def rgb_to_od(image: Array, clip = True) -> Array:
