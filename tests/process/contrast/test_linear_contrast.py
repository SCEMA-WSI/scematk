...
class TestLinearContrast:

    def test_linear_contrast_image(self):
        image = read_zarr_ubimg(
            "./tests/_testdata/raw_img/image.zarr", "./tests/_testdata/raw_img/meta.json"
        )
        contrast_image = LinearContrast(2).run(image).image.compute()
    ...
    def test_linear_contrast_meta(self):
        image = read_zarr_ubimg(
            "./tests/_testdata/raw_img/image.zarr", "./tests/_testdata/raw_img/meta.json"
        )
        contrast_image = LinearContrast(2).run(image)
