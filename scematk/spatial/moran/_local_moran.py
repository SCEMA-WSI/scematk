...
class LocalMoran():
    ...
    def fit(self, data: dd.DataFrame, value_col: str, x_col: str, y_col: str, label_col: str = 'Meta_Global_Mask_Label') -> dd.DataFrame:
        ...
        z2 = z2.sum()
        n = W.shape[0]
        local_moran = ((n/z2) * z * (W @ z[:,None]).sum(axis=1)).to_dask_dataframe().repartition(npartitions=data.npartitions)
        df = data[[label_col]]
